...
AppController.prototype.exportBlockLibraryToFile = function() {
  ...
  // Get file name.
  var filename = prompt('输入要保存您的块库的文件名，“library.xml”。', 'library.xml');
...
AppController.prototype.assignBlockFactoryClickHandlers = function() {
  ...
  document.getElementById('files').addEventListener('change',
      function() {
        // Warn user.
        var replace = confirm('这个导入的模块库将会取代你当前的模块库。');
        if (replace) {
...
AppController.prototype.initializeBlocklyStorage = function() {
  ...
  '使用此公共链接共享您的块。如果没有一年使用，我们将删除它们。它们与您的帐户无关，并按Google的隐私政策处理。请务必不要包含任何私人信息。:\n\n%1';
  BlocklyStorage.HASH_ERROR =
      '抱歉，"%1"与任何保存的Blockly文件都不对应。';
...
AppController.prototype.confirmLeavePage = function(e) {
  ...
  if ((!BlockFactory.isStarterBlock() &&
    ...
    var confirmationMessage = '您将会丢失所有未保存的更改。您确定要退出此页面吗？';
